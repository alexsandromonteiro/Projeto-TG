...
if(isset($_POST['request'])) {
    ...
    $query = "SELECT id_itens_pedido,
    composicao, descricao_item, preco, tipo_item, imagem, tipo_item
    FROM itens WHERE tipo_item = 'bebida'";
    $resulConsultaBebida = mysqli_query($conexao, $query);
    $count = mysqli_num_rows($resulConsultaBebida);
}

?>
<?php
while($row = mysqli_fetch_assoc($resulConsultaBebida)) {
            ?>
